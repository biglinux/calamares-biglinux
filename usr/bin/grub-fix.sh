...
sed -i "s|GRUB_CMDLINE_LINUX_DEFAULT=\"|GRUB_CMDLINE_LINUX_DEFAULT=\"$(cat /proc/cmdline | sed 's|.*misolabel=[[:alnum:]_-]*||g;s|driver=nonfree||g;s|driver=free||g;s|video=efifb:nobgrt|fbcon=nodefer video=efifb:nobgrt|g;s|rdinit=/vtoy/vtoy quiet||g;s|rdinit=/vtoy/vtoy||g') |g" $*
