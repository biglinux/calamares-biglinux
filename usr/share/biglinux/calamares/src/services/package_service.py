...
class Package:
    ...
    def __init__(self, name: str, icon: Optional[str] = None, 
                 installed: bool = False, selected: bool = True):
        self.name = name
...
class PackageService:
    ...
    def _load_minimal_packages(self):
        ...
        try:
            ...
            if data and 'packages' in data:
                ...
                self.logger.debug(f"Loaded {len(self._minimal_packages)} minimal packages")
            else:
    ...
    def _refresh_installed_packages(self):
        ...
        try:
            ...
            self.logger.debug(f"Found {len(self._installed_packages)} installed packages")
    ...
    def get_packages_for_removal(self, selected_packages: List[str]) -> List[str]:
        ...
        for pkg_name in selected_packages:
            if (validate_package_name(pkg_name) and 
                self.is_package_installed(pkg_name) and
                pkg_name in self._minimal_packages):
...
"""
            ...
            self.logger.info(f"Created package removal config with {len(packages_to_remove)} packages")
            return True
    ...
    def validate_package_selection(self, selected_packages: List[str]) -> Dict[str, List[str]]:
        """
